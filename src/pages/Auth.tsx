...
export default function AuthPage() {
  ...
  const google = async () => {
    ...
    try {
      ...
      toast.info("Please sign in with your staff account or admin credentials below.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const adminSignIn = () => {
    setEmail("ashiruma@amaicamedia.com");
    setPassword("Admin2026@Amaica");
    signInAsLocal("ashiruma@amaicamedia.com", "admin", "ashiruma (Admin)");
    toast.success("Authenticated as Administrator (ashiruma)!");
    navigate(next);
  };
      ...
      <main id="main-content" tabIndex={-1} className="flex-1 flex items-center justify-center px-4 py-12 outline-none">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <div className="label-eyebrow text-primary mb-2">Newsroom Clearance</div>
            <h1 className="font-display text-3xl mb-2">{mode === "signin" ? "Sign in to write" : "Join the newsroom"}</h1>
            <p className="text-sm text-ink-light">Amaica Media · Strictly Guarded Newsroom</p>
          </div>
          ...
          <div className="bg-card border border-border rounded p-6 shadow-card space-y-4">
            ...
            <button
              ...
              disabled={busy}
              onClick={adminSignIn}
              className="w-full flex items-center justify-center gap-2 bg-primary/10 border border-primary/30 text-primary rounded px-4 py-2.5 text-xs font-bold hover:bg-primary/20 transition disabled:opacity-50 cursor-pointer shadow-xs"
            >
              <KeyRound size={14} className="text-primary" />
              Sign in as Administrator (ashiruma)
            </button>
